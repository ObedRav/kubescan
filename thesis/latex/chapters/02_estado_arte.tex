...
La aplicación de técnicas de aprendizaje automático (\ac{ML}) a la predicción de
...
\toprule
Algoritmo & \shortstack{Sin norma-\\lización} & Desequilibrio & Prob.\ calibrada & Observación \\
\midrule
Random Forest & \checkmark & \checkmark & Buena & Adoptado (Capa~1) \\
Árbol de Decisión & \checkmark & Parcial & Pobre & Alta varianza \\
\ac{SVM} (RBF) & No & Parcial & Requiere Platt & Entrenamiento O(n\textsuperscript{2})--O(n\textsuperscript{3}); viable pero menos eficiente \\
Red Neuronal (\acs{MLP}) & No & Con pesos & Variable & Sensible a la escala de entrada \\
\textit{k}-NN & No & No & Estimada & Costoso en inferencia \\
\bottomrule
\end{tabular}
\fuentepropia
\end{table}
