...
del problema: el análisis estático delimita qué constituye una misconfiguración,
...
misconfiguraciones mediante reglas estáticas: no incluye ningún modelo de
...
La noción de un índice unificado de misconfiguraciones (\textit{unified
...
como el de la Capa~1 del presente sistema; la fuente original emplea esa expresión
descriptiva sin acuñar una sigla, por lo que la abreviatura \acs{UMI}\acused{UMI}
que se usa en el resto de este documento es una convención propia. Su propuesta, que incorpora un modelo de
...
implementación en \textit{scikit-learn} \citep{Pedregosa2011}. En la literatura, el
...
y modelos de mezclas esperadas, sin reportar valores de F1 macro ni resultados de
\textit{Random Forest}. No constituye, por tanto, una revisión del rendimiento de
\textit{Random Forest} sobre ese conjunto ni permite derivar de él un rango de
...
espacio, el \textit{Random Forest} resulta especialmente adecuado para el problema
...
superiores a los clasificadores tabulares clásicos sobre las re-publicaciones en
...
(Capítulo~\ref{cap:herramienta}) confirma que \texttt{TRUE\_HOST\_PID} —la flag de escape
