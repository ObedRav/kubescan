...
\textbf{L2 — Cobertura del escaneo de características extendidas.} Seis de las
siete características extendidas —todas salvo \texttt{HOSTPATH\_MOUNT}, poblada
en las 2.827~filas— dependen de que el manifiesto de origen haya sido escaneado:
el análisis sintáctico propio las obtiene para el 32,6\,\% de las filas, la
salida de \textit{Checkov} las rellena para otro 41,6\,\% y las 731~filas
restantes (25,9\,\%) se imputan por la mediana de columna, diluyendo su señal
(Sección~\ref{sec:construccion_dataset}). La causa de esa carencia no reside en
la disponibilidad de los datos: el registro de descarga del corpus de Rahman
et~al.\ documenta 1.985~manifiestos obtenidos sobre los 2.039 referenciados
(97,4\,\%), y los 54~fallos se reparten entre 33~respuestas \texttt{404 Not
Found} y 21~respuestas \texttt{HTTP 403} emitidas por el servidor de origen. El
origen real fue un defecto del \textit{pipeline} de extracción, ya corregido: la
resolución de rutas de \texttt{scan\_security\_tools.py} reconocía únicamente el
prefijo \texttt{GITLAB\_K8S\_REPOS\_RAW\_UNFILTERED/}, mientras que las filas del
conjunto tabular emplean \texttt{GITLAB\_REPOS/}, de modo que las 396~filas
procedentes de GitLab fallaban en la extracción de clave, nunca se escaneaban y
recaían en la imputación por mediana. Tras corregir el defecto se recuperaron
350 de esas 396~filas, y el reentrenamiento de la Capa~1 sobre el conjunto
reescaneado deja la F1 macro inalterada (0,9946 antes y después,
\(\Delta\)\,=\,0,0000), con una perturbación del \texttt{risk\_score} por fila de
media 0,0030, mediana 0,0013, percentil~95 de 0,0117 y máximo 0,1104, y con solo
2 de las 2.827~filas desplazándose más de 0,10. La brecha residual la componen
335~filas de GitHub aún sin resolver y 46~manifiestos de GitLab genuinamente
inaccesibles en origen —381~filas en total, el 13,5\,\% del corpus—, que siguen
resolviéndose por mediana. El efecto medido sobre la Capa~1 es, por tanto, nulo,
y la perturbación inducida en las características de nodo queda un orden de
...
exigiría reconstruir los grafos de clúster y reentrenar el conjunto completo.
...
características extendidas que todavía se imputan por mediana en el 13,5\,\% de
las filas frente a las obtenidas por escaneo directo del manifiesto, si bien el
reescaneo de las filas recuperadas dejó la F1 macro de la Capa~1 sin variación. L9 añade una restricción del mismo orden:
...
independientes por pliegue (1--3), no el número nominal de cadenas. Por su parte,
