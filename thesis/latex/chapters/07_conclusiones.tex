...
(\(\sigma\)\,=\,0,033 en P@5, Sección~\ref{sec:multisemilla}). El efecto sobre
las Capas~2 y~3 está acotado por construcción. El constructor del grafo toma las
características de nodo directamente de las columnas de \texttt{FEATURE\_COLS} e
imputa como 0 toda celda vacía, sin consultar las columnas de diagnóstico
\texttt{checkov\_*}: el relleno mediante \texttt{CHECKOV\_FILL} opera únicamente en
la carga de la Capa~1 (Sección~\ref{sec:construccion_dataset}). Las seis
características toman en consecuencia valor 0 en el vector de nodo de las
1.906~filas procedentes de Rahman et~al., presentes en 62 de los 599~grafos
originales, y la cobertura de escaneo no altera los grafos, sus etiquetas ni las
salidas de las Capas~2 y~3. Esa invariancia acota la limitación y desplaza su
coste: seis de las 25~banderas del vector de nodo carecen de señal para ese
subconjunto de manifiestos.
...
banderas binarias que ocupan los índices 0--24, y está vacío —imputado como 0— en
los 517~clústeres procedentes de repositorios de seguridad adicionales, es decir en
el 86\,\% de los grafos. Una ablación sobre el conjunto de test que anula el
índice~25 en los 86~grafos deja las métricas inalteradas (P@1\,=\,1,00,
P@3\,=\,1,00, P@5\,=\,0,80, \ac{FPR}\_clean\,=\,0,00; F1 macro 0,8825, con
matriz de confusión idéntica). El resultado no permite concluir que la Capa~1 sea
prescindible para la Capa~2, sino que su aportación no ha llegado a evaluarse: el
canal previsto para transmitirla no transportaba la salida del clasificador.

La Capa~3 hereda la misma discrepancia. El término \(\overline{\text{risk}}(C)\)
de la Ecuación~\ref{eq:ensemble} se calcula, durante la optimización de los pesos,
como media del atributo nodal~25 sobre los nodos del clúster, de modo que el
Algoritmo Genético ajustó \(w_{\text{rf}}\) —el mayor de los tres pesos, 0,517—
frente a una señal nula en el 86\,\% de los grafos. En el sistema distribuible ese
mismo término se evalúa sobre la media de las probabilidades que devuelve el
clasificador, señal de naturaleza distinta a la empleada al ajustar el peso que la
pondera. La magnitud del efecto de esa discrepancia sobre el \textit{ranking} en
inferencia no se ha cuantificado: establecerlo exigiría reoptimizar los pesos sobre
predicciones \textit{out-of-fold} construidas con la salida del clasificador, tarea
recogida en TF9.

\subsection{Amenazas a la validez}
\label{sec:amenazas_validez}

Las limitaciones L1--L10 se clasifican a continuación en cuatro categorías de
...
variación. En la Capa~2 la imputación es a 0 y alcanza a las 1.906~filas de
Rahman et~al., pero no depende de la cobertura de escaneo, por lo que no
compromete la comparabilidad entre configuraciones. L9 añade una restricción del
mismo orden: al haberse fijado los hiperparámetros a priori, las comparaciones
entre configuraciones se establecen sobre un único punto del espacio de diseño y
no sobre el óptimo de cada alternativa. L10 acota, por último, la interpretación
de la arquitectura: la contribución de la Capa~1 a la Capa~2 no está medida,
porque el atributo nodal previsto para transmitirla está vacío en el 86\,\% de
los grafos.
...
las limitaciones L1--L10 de la Sección~\ref{sec:limitaciones}, referenciadas por su
...
atributo nodal~25 con la salida del clasificador de la Capa~1 para todas las fuentes
del corpus —no sólo para el subconjunto de Rahman et~al.— y repetir la ablación de
L10 permitiría medir por primera vez si la señal por manifiesto aporta capacidad
predictiva a la clasificación estructural, cuestión que el diseño actual deja
abierta. La misma corrección habilitaría reoptimizar los pesos del \textit{ensemble}
sobre predicciones \textit{out-of-fold} homogéneas con las de inferencia, eliminando
la discrepancia que L10 documenta en la Capa~3.
