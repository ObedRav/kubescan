...
de kubescan. Los requisitos funcionales y no funcionales se derivan del
...
El diseño de kubescan parte de un análisis de requisitos derivado tanto de
...
\begin{description}
  \item[RF-1 — Detección de configuraciones erróneas individuales.] El sistema debe cumplir
    dos obligaciones separables sobre cada manifiesto \acs{YAML}. La primera es
    clasificarlo como seguro o mal configurado —\textit{misconfigured} en la
    nomenclatura del modelo\mbox{—.} La segunda es producir un
    índice de severidad en tres niveles (limpio, bajo-medio, alto-crítico)
    que ordene las configuraciones erróneas detectadas por gravedad.
    ...
    (Checkov, kube-linter) ya cubren esta capacidad por
  ...
  \item[RF-2 — Detección de cadenas de ataque.] El sistema debe clasificar un conjunto
    ...
    \textit{isolated} (configuraciones erróneas sin cadena viable) o
    ...
    \textit{argmax} se consigna como métrica descriptiva del comportamiento del
    ...
    grafo basadas en reglas declarativas que incorpora Checkov se limitan
  ...
  \item[RF-3 — Interfaz de línea de comandos.] El sistema debe exponer un comando
    ...
    \texttt{kubescan live} para análisis de clústeres activos mediante \texttt{kubectl}.
  ...
  \item[RF-5 — Priorización de riesgos.] El sistema debe producir una clasificación
    ...
    coherente con el objetivo de diseño y no como una garantía estadística.
    \textit{Justificación:} un equipo de seguridad revisa manualmente un número
    reducido de alertas por jornada; un \textit{ranking} que sitúe las cadenas reales en
...
\begin{description}
  ...
  \item[RNF-2 — Reproducibilidad.] Los modelos entrenados deben almacenarse como
    ...
    no es fiable como \textit{gate} de \acs{CICD} —una comprobación que bloquea la
    entrega si falla\mbox{—,} donde el mismo manifiesto debe
...
después con una evidencia experimental u operativa determinada. La
Tabla~\ref{tab:trazabilidad} del Anexo~\ref{ap:repositorio} recoge esa
correspondencia, indicando para cada
...
verificación operativa de la Sección~\ref{sec:verificacion_requisitos}.
...
clúster activo (KubeHound \citep{KubeHound2023}, IceKube
\citep{IceKube2023}) solo pueden operar con posterioridad al despliegue y
...
riesgo específica de cada manifiesto. La elección de Random Forest para
...
A las tres capas se añade un cuarto componente arquitectónico: la restricción
...
configuraciones erróneas encontradas; y el análisis de vulnerabilidades a nivel de
...
análisis estructural que aporta kubescan.
...
La selección de métricas de evaluación responde a los requisitos operativos del
sistema y a las características del conjunto de datos. Cada métrica se corresponde con el
...
importancia a las clases minoritarias (configuraciones erróneas críticas) que a las
mayoritarias (manifiestos seguros). Esta propiedad es fundamental en conjuntos de datos de
...
que predice siempre «seguro» obtendría una exactitud del 56,4\,\% pero F1 macro
del 36,0\,\%.
...
\end{equation}
\addequationtoindex{Precisión@\textit{k} normalizada}
...
alineada con el caso de uso operativo: un equipo de seguridad que audita los
