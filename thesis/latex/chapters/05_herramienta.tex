...
no aportan información estructural adicional al grafo\mbox{—;} ver
...
Movimiento lateral \acs{SA}\acused{SA} & 2 & Dirigida (lateral\(\to\)mismo \textit{namespace}) & Bandera de movimiento lateral \\
Co-espacio de nombres (\texttt{SEMANTIC\_NS}) & 3 & Bidireccional & Mismo \textit{namespace} K8s \\
Escalada \ac{RBAC} & 4 & Dirigida (elevado\(\to\)mismo \textit{namespace}) & \acs{SA} vinculado a rol privilegiado \\
\bottomrule
\end{tabular}
\fuentepropia
\end{table}
...
escape activan —salvo \texttt{badpods\_nothing-allowed}, que no activa ninguna\mbox{—,}
...
papel en el grafo —detección de escape (ESC) o de movimiento lateral (LAT)\mbox{—,} se
