...
trabajo, no reportan modelos de clasificación supervisada sobre el mismo
conjunto: el objetivo de su trabajo era la construcción del dataset y el
análisis de la densidad de misconfiguraciones, no la predicción, por lo que no
ofrece una cifra de referencia directa. La línea de base aplicable procede en
su lugar de la literatura de \acs{IDS} sobre UNSW-NB15 \citep{Moustafa2015},
donde clasificadores basados en \textit{Random Forest} típicamente alcanzan F1
macro entre 0,85 y 0,93; ese rango es el que motiva el umbral objetivo
F1\,>\,0,85 adoptado en este trabajo (OE2). Frente a esa línea de base, el
resultado de la Capa~1 (F1 macro\,=\,0,9946, \ac{AUC}-\ac{ROC}\,=\,0,9986;
Sección~\ref{sec:rf_importance} y siguientes) se sitúa por encima del extremo
superior del rango de referencia. Esta separación casi perfecta es consistente
con la naturaleza del problema: las misconfiguraciones en manifiestos
\acs{YAML} son mayoritariamente discretas y acumulativas (más flags activas
...
No existe ningún trabajo previo que aborde la clasificación supervisada de
cadenas de ataque en clústeres \textit{Kubernetes} mediante \acs{GNN}s, por lo
que la Capa~2 no dispone de una línea de base directamente comparable. Las
referencias más cercanas son los trabajos de \acs{GNN}s para \ac{IDS} en red,
como \textit{E-GraphSAGE} \citep{Lo2022}, que reporta mejoras del 2--8\,\% en F1
sobre clasificadores tabulares en el dataset UNSW-NB15; y la literatura sobre
conjuntos de grafos pequeños (n\,<\,200), que identifica la escasez de datos
como el principal factor de variabilidad en \acs{GNN}s para ciberseguridad y
recomienda validación cruzada estratificada como estándar de evaluación para
conjuntos de este tamaño \citep{GNNSurvey2024} — la metodología adoptada en
este trabajo. Frente a esa referencia, la ventaja del componente estructural
observada en este trabajo es mayor de lo reportado por \textit{E-GraphSAGE} y
se mide sobre el conjunto de test genuinamente no visto: P@5 pasa de 0,40
(\ac{RF}-sólo) a 0,80 (configuraciones con \ac{GNN}) y P@1 de 0,00 a 1,00
...
(\(\sigma\)\,=\,0,160 para P@5) es coherente con la variabilidad esperada según
\citeauthor{GNNSurvey2024} \citeyear{GNNSurvey2024} para conjuntos de este
tamaño, y en este trabajo tiene además una causa estructural identificada: el
particionado por familias concentra las cadenas de validación de cada pliegue
en 1--3 familias independientes (Sección~\ref{sec:layer2}), de modo que la
desviación estándar entre pliegues mide en gran parte la dispersión de
dificultad entre familias y no inestabilidad del modelo, como confirma el
análisis multi-semilla (Sección~\ref{sec:multisemilla}).
...
reales, genuinamente no vistos durante el desarrollo; dado el tamaño reducido de
...
Esta cifra es la bondad global de la herramienta porque resume, en un único
indicador operacional, la salida final que recibe el operador tras pasar por las
tres capas: un P@1 perfecto significa que la primera posición del
\textit{ranking} es siempre una cadena de ataque real cuando existe, un P@3
perfecto extiende esa garantía a las tres primeras posiciones, y un
\ac{FPR}\_clean nulo significa que ningún clúster limpio compite nunca por la
atención del operador. La Sección~\ref{sec:layer3} muestra que, sobre este
...
puntos débiles de los demás (la Capa~1 sola colapsa el ranking y la señal de
...
RF-1 (detección de misconfiguraciones individuales) queda verificado por los
...
(F1~\textit{attack\_chain}\,=\,0,667, Sección~\ref{sec:gnn_por_clase}); el
...
directorio de manifiestos de prueba con flags de escape conocidas, la salida
