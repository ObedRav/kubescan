...
\citet{Rahman2022}, cuyos datos forman la base del presente
...
clasificadores tabulares clásicos sobre las republicaciones en formato NetFlow
...
lo que indica que la información estructural del grafo es decisiva para identificar
...
multisemilla (Sección~\ref{sec:multisemilla}) matiza esa lectura: cuatro de los
...
características de riesgo de severidad media (por ejemplo, \texttt{NO\_RUN\_AS\_NON\_ROOT}
...
ordenamiento resultante es, en cambio, coherente con la matriz de confusión de
...
de ablación que elimina \texttt{total\_misconfigs} produce F1 idéntico y la misma
...
las etiquetas heterogéneas del resto de fuentes\mbox{—,} no una noción de
...
\textit{high-critical}\mbox{—,} lo que rebaja su precisión a 0,9615. La clase
...
El objetivo de diseño de \textit{ranking} es P@5\,>\,0,70 y el umbral
...
de 0,68\,\(\pm\)\,0,033, con el objetivo dentro de una desviación típica de la
media\footnote{Las desviaciones típicas de las métricas del modelo
se expresan como desviación poblacional (\(\text{ddof}=0\)), coherente con la
...
literatura.} (Sección~\ref{sec:multisemilla}).
...
selección —no al corpus ni al protocolo de particionado\mbox{—,} y que revertir a
...
—un tamaño muestral efectivo menor que el número nominal de cadenas\mbox{—,} lo que
...
frente a 0,800\mbox{—,} pero esa coincidencia agregada oculta un desacuerdo apreciable
...
no evidencian robustez de la arquitectura frente a la profundidad; lo que
muestran es que la profundidad no determina el nivel medio de las métricas sobre
este corpus. Se
...
resortear también las particiones confundiría la varianza del modelo con la
...
métricas de test del despliegue son estables —P@1\,=\,1,00 y
P@3\,=\,1,00 con las tres semillas, y P@5 entre 0,80 y 1,00\mbox{—,} de modo que la
...
determinista (Sección~\ref{sec:capa3_diseno}, 231~evaluaciones): el vértice de
señal de escape pura (\(w_{\text{esc}}=1\), \textit{objective}\,=\,0,86),
lo que confirma que la función objetivo tenía un óptimo global único y alcanzable, no
...
4~\textit{isolated} y 3~limpios\mbox{—,} de modo que las métricas de \textit{ranking} no
...
intervalos de confianza amplios (\textit{bootstrap} de 10.000 remuestreos, 95\,\%):
P@5 \([0{,}60;\ 1{,}00]\), \ac{FPR}\_clean \([0{,}00;\ 0{,}40]\) y F1~macro
\([0{,}66;\ 1{,}00]\). Los valores puntuales deben interpretarse como
\textit{coherentes con} los objetivos de diseño, no como demostraciones
...
multinodo \texttt{kubernetes-extras}) es una cadena de 4~nodos cuya
...
de mayor carga\mbox{—,} por lo que la extrapolación a 500~manifiestos sitúa el tiempo
...
individuales de 100, 80 y 85), coherente con la media de referencia del sector
...
de la media abarca \([62{,}5;\ 114{,}2]\) —su extremo superior rebasa el máximo
de la escala, artefacto de la aproximación normal con una muestra tan reducida— y
contiene ese valor de referencia, por
...
concretas hay detrás»\mbox{—,} si bien solo uno de ellos la trasladó a la escala
