...
reportar F1 macro ni resultados de \textit{Random Forest}
...
\textit{Kubernetes} mediante \acs{GNN}s, por lo que la Capa~2 no dispone de una
...
\citep{Lo2022}, que reporta tasas de detección superiores a las de
...
\citep{GNNSurvey2024}. Conviene precisar que esta última no establece un régimen
...
disyunción de todas las \textit{flags} binarias, en cambio, se desploma a
...
característica derivada: su señal es recuperable a partir de las flags que la
componen.
...
define como la presencia de al menos una misconfiguración de un subconjunto
...
misconfiguración independiente de dicha regla. Esta limitación, análoga a la que
...
flags de escape activas como \texttt{TRUE\_HOST\_PID} o
...
propiedad relevante desde el punto de vista de seguridad operacional: en el peor
...
aporta +0,12~puntos de P@5 con selección por F1 macro y sólo +0,04 con selección
por P@5, mientras que seleccionar los \textit{checkpoints} por la métrica
primaria de ranking en lugar de por F1 macro aporta +0,20~puntos bajo
...
la configuración final se reentrenó con tres semillas (7, 42, 123; Tabla~\ref{tab:multiseed}
...
los cinco pliegues de validación cruzada. El registro de la validación cruzada
conserva únicamente esas dos métricas por pliegue y la exactitud agregada, sin
matrices de confusión individuales, por lo que el análisis por clases se apoya
exclusivamente en el conjunto de test. En él, la matriz de confusión del ensemble
...
clasificación como para el ranking de cadenas. La comparación 2 vs.\ 3~capas es
...
de sólo 0,003. Dos modelos que difieren en esa magnitud sobre pliegues concretos
...
F1 macro y por coherencia con el resto de resultados reportados. La
...
Como contraste externo, se ejecutó \textit{Checkov} \citep{Checkov2021} sobre
...
El alcance de este contraste es limitado y conviene delimitarlo. La ejecución de
\textit{Checkov} vuelca sus identificadores de comprobación en columnas de
diagnóstico \texttt{checkov\_*} independientes, que no alimentan las
características de la Capa~1: éstas proceden del análisis sintáctico propio del
\acs{YAML} descrito en el Capítulo~\ref{cap:herramienta}. Sobre esas mismas
1.175~filas, además, las características homólogas del extractor propio
(\texttt{NO\_RUN\_AS\_NON\_ROOT} y \texttt{NO\_READ\_ONLY\_ROOT\_FS}) están
ausentes en su totalidad, de modo que no es posible cotejar detección con
detección manifiesto a manifiesto ni cuantificar una tasa de acuerdo entre ambas
fuentes. El resultado debe interpretarse, en consecuencia, como una corroboración
de cobertura y procedencia —ambos conjuntos de características codifican los
mismos controles de configuración segura de referencia para \textit{Kubernetes},
obtenidos por implementaciones distintas— y no como una validación externa
independiente de las predicciones del modelo.
...
rapidez del análisis. La debilidad más recurrente en las respuestas abiertas es
...
concretas hay detrás»—, si bien sólo uno de ellos la trasladó a la escala
