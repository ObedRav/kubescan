...
Este capítulo evalúa el sistema \textit{kubescan} completo. Se parte de una
línea de base tomada del estado del arte, frente a la cual se contrastan los
resultados de cada capa; a continuación se reportan dichos resultados en el
mismo orden que las fases de entrenamiento descritas en el
Capítulo~\ref{cap:herramienta}, se sintetiza una métrica de proceso completo que
resume el comportamiento extremo a extremo del sistema, y se verifica el
cumplimiento de los requisitos funcionales y no funcionales definidos en el
Capítulo~\ref{cap:requisitos}, requisito a requisito. Las métricas empleadas se
justifican en detalle en la Sección~\ref{sec:metricas}.
...
Antes de reportar los resultados propios, esta sección fija la línea de base
...
del problema: las misconfiguraciones en manifiestos \acs{YAML} son mayoritariamente discretas
...
\textit{Kubernetes} mediante \acs{GNN}s. Las referencias más cercanas son los
trabajos de \acs{GNN}s para \ac{IDS} en red, como \textit{E-GraphSAGE}
...
factor de variabilidad en \acs{GNN}s para ciberseguridad, y recomiendan
...
reportan primero las métricas de clasificación binaria y la importancia de las
...
que refleja la densidad de misconfiguraciones del manifiesto. En la matriz de
confusión de la Figura~\ref{fig:rf_confusion} (566~muestras), los 3~falsos
positivos corresponden a manifiestos seguros con alta densidad de
características de riesgo, y no existe ningún falso negativo.
...
matriz de confusión, confirmando que el modelo no depende de esta característica
derivada y que no existe fuga de datos circular.
...
igualmente conservador. El modelo de severidad se utiliza en el informe de salida
de \textit{kubescan} para proporcionar al operador una descripción cualitativa
...
válidas y reproducibles. Esta evolución se documenta explícitamente porque es
también, en sí misma, una contribución metodológica del trabajo.
...
Las otras tres configuraciones quedan por debajo de P@5\,>\,0,70 con márgenes
distintos, lo que confirma que ambos factores —función de pérdida y criterio de
selección— deben acertarse simultáneamente para alcanzar el objetivo.
...
\begin{tabular}{@{}crrc@{}}
\toprule
Semilla & P@5 (\acs{CV}) & F1 macro (\acs{CV}) & Test P@1/P@3/P@5 \\
\midrule
...
con 3 de las 5~cadenas correctamente clasificadas). La Figura~\ref{fig:structural_gate}
compara las métricas de ranking del conjunto de test con y sin la restricción:
descartar los clústeres de un solo nodo del \textit{ranking} eleva P@1 de 0,00 a
1,00 y P@5 de 0,60 a 0,80.
...
literatura sobre \acs{GNN}s aplicadas a grafos pequeños heterogéneos documenta de forma
...
los 1.175~manifiestos cuyo \acs{YAML} descargado resultó analizable por la
...
Las secciones anteriores evalúan cada capa por separado; esta sección destaca la
métrica que resume el comportamiento del sistema completo de extremo a extremo,
tal como lo experimenta un operador que confía en el \textit{ranking} final. Esa
métrica es el conjunto Precisión@k / \ac{FPR}\_clean del \textit{ensemble} sobre
el conjunto de test, ya reportado en la Sección~\ref{sec:layer3} y resumido en
la Figura~\ref{fig:ensemble_patk}: P@1\,=\,1,00, P@3\,=\,1,00, P@5\,=\,0,80 y
\ac{FPR}\_clean\,=\,0,00 sobre 86~grafos con 5~cadenas de ataque de repositorios
reales, genuinamente no vistos durante el desarrollo.

Esta cifra es la bondad global de la herramienta porque integra las tres capas
en un único indicador operacional: un P@1 y un P@3 perfectos significan que las
dos y las tres primeras posiciones del \textit{ranking} son siempre cadenas de
ataque reales cuando existen, mientras que un \ac{FPR}\_clean nulo significa que
ningún clúster limpio compite nunca por la atención del operador de seguridad.
La combinación de ambos valores es lo que hace viable el caso de uso definido en
RF-5 (Sección~\ref{sec:requisitos}): priorizar un número reducido de alertas de
forma que la revisión manual de un equipo de seguridad se concentre
exclusivamente en riesgo real. Los intervalos de confianza de esta cifra, dado
el tamaño del conjunto de test, se reportan junto con la métrica en la
...
Esta sección verifica, requisito a requisito, el cumplimiento de los requisitos
funcionales y no funcionales definidos en el Capítulo~\ref{cap:requisitos}.
Complementa las métricas predictivas de las secciones anteriores con evidencia
de que el sistema es efectivamente utilizable como herramienta de línea de
comandos, y cierra con la evaluación de usabilidad y aplicabilidad realizada con
usuarios expertos.

\subsection{Requisitos funcionales}

RF-1 y RF-2 (detección de misconfiguraciones individuales y de cadenas de
ataque) quedan verificados por los resultados de las Capas~1 y~2 reportados en
las Secciones~\ref{sec:layer2} y anteriores, y se confirman además mediante un
test de integración de extremo a extremo: dado un directorio de manifiestos de
prueba con flags de escape conocidas, la salida del comando
\texttt{kubescan scan} produce el veredicto esperado (\texttt{ATTACK\_CHAIN})
con \textit{ensemble\_score} por encima del umbral de clasificación
(Sección~\ref{sec:calidad}). RF-5 (priorización de riesgos) queda verificado por
la métrica global de la Sección~\ref{sec:metrica_global}. RF-3 y RF-4 (interfaz
de línea de comandos y formatos de salida) no admiten una métrica cuantitativa
dedicada y se verifican operacionalmente a continuación.

\paragraph{RF-3 — Interfaz de línea de comandos.} Los dos comandos definidos en
la Sección~\ref{sec:implementacion} (\texttt{kubescan scan <dir>} y
\texttt{kubescan live}) se ejercitaron exitosamente sobre los casos de uso
documentados en el Apéndice~\ref{ap:guia_uso}, incluyendo el análisis de un
directorio de manifiestos y la consulta directa a un clúster en ejecución.
...
\subsection{Evaluación de usabilidad y aplicabilidad con usuarios expertos}
\label{sec:usabilidad}

Además de la verificación de requisitos anterior, se realizó una evaluación de
...
ayuda en dos casos. La tasa de éxito global del 90\,\% (Tabla~\ref{tab:usab_tareas})
...
\subsubsection{Aplicabilidad y análisis cualitativo}
...
punto más bajo, A3\,=\,3,7. El análisis cualitativo de las respuestas abiertas es coherente con estas cifras.
...
\subsubsection{Amenazas a la validez de la evaluación}
