...
OE4, OE1, OE5, OE3 y OE6 definidos en la Sección~\ref{cap:objetivos}; C3 no cierra un
objetivo propio, sino que aporta evidencia complementaria sobre el impacto operacional
del resultado de C2. Las conclusiones principales son:
...
(\ac{FPR}\_clean\,=\,0,00), y con estas métricas de test estables a través de
las tres semillas de entrenamiento evaluadas. Parte esencial del resultado es la
...
duplicados de plantilla (siete \texttt{badpods\_*}, nueve \texttt{datadog\_*})
...
vistas, una vara de medir más exigente y más honesta que la de las fases
anteriores, cuyas métricas estaban infladas por esta contaminación.
...
integración en pipelines \acs{CICD}, y garantiza reproducibilidad de la inferencia
mediante la resolución determinista de \textit{checkpoints} (sujeto a la variación
entre \textit{backends} de cómputo documentada en L6). El prototipo implementado
en este
...
5)—, con una tasa de éxito del 100\,\% en las tareas de instalación, escaneo e
interpretación. El principal punto de mejora señalado por los participantes es la
...
el 47,4\,\% de las filas con \ac{YAML} descargado y se imputan por la mediana de
...
limitaciones L1--L7 se organizan del siguiente modo:

\textbf{Validez de constructo} (¿miden las métricas lo que se pretende medir?):
L6 cuestiona si el conjunto de métricas reportado en esta memoria (\textit{backend}
MPS/CPU) mide el mismo constructo que mediría un \textit{backend} CUDA de
producción, dado el no determinismo documentado en las operaciones de
\textit{scatter}. L5 cuestiona si la etiqueta de grafo (derivada de reglas sobre
