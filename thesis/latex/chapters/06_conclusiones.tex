...
\textit{Random Forest} alcanza F1 macro\,=\,0,9935 y \ac{AUC}\,=\,0,9980 en test, superando
el umbral objetivo del 0,85 en 14,4 puntos porcentuales. El modelo de severidad de
tres clases logra F1\,=\,0,9541 en la clase \textit{high-critical} (4~errores sobre
56~muestras de test, incluyendo 1~manifiesto crítico clasificado como \textit{clean}),
el resultado más débil de los tres clasificadores de severidad pero aun así por
encima de 0,95.
...
\textbf{C5 — El esquema de cinco tipos de arista amplía la cobertura de escalada de
...
cadenas de ataque queda como trabajo futuro.
...
integración en pipelines CI/CD, y garantiza reproducibilidad de la inferencia
mediante la resolución determinista de \textit{checkpoints} (sujeto a la variación
entre \textit{backends} de cómputo documentada en L7). El prototipo implementado
en este
...
participa en este ajuste. Un diagnóstico del algoritmo descartó que su
estancamiento inicial en pesos casi uniformes se debiera, como se supuso en un
primer momento, a una meseta plana en la función de \textit{fitness}: el cruce por
promedio aritmético y la mutación gaussiana local no podían alcanzar los vértices
del símplex de pesos, no porque esos vértices fueran indistinguibles en la función
objetivo. Al corregir la exploración —sembrando los vértices y aristas del símplex
en la población inicial y añadiendo una mutación de salto de frontera— el
\ac{GA} converge de forma determinista, en 5~semillas independientes, al mismo
óptimo que la búsqueda en rejilla: puntuar únicamente por la señal binaria de
escape (\textit{objective}\,=\,0,86 frente a 0,72 de los pesos equilibrados). La
limitación real no es, por tanto, una meseta de \textit{fitness} sino la
identificabilidad del criterio de selección: el óptimo \acs{OOF} no generaliza al
conjunto de test, donde el colapso a una señal binaria produce un ranking por
empates que falla estructuralmente ante cualquier cadena de ataque sin bandera de
escape (Sección~\ref{sec:layer3}). Los pesos equilibrados, pese a no ser el óptimo
\acs{OOF}, se conservan para el despliegue porque no exhiben esa falla
estructural; aun así, el tamaño de ambos conjuntos —81~grafos \acs{OOF} y
15~de test— limita la confianza estadística de cualquier selección de pesos
basada en datos, como reflejan los intervalos de confianza reportados.
...
\textbf{L7 — No determinismo de CUDA en las operaciones de \textit{scatter} del
\ac{GNN}.} Al replicar el proceso de entrenamiento de la Capa~2 sobre una GPU
NVIDIA A100 (CUDA) en lugar del \textit{backend} MPS/CPU empleado para los
resultados de esta memoria, se observó una variación no trivial en las métricas de
validación cruzada (F1 macro entre 0,845 y 0,882 frente a 0,917 en el \textit{backend}
original; P@5 entre 0,77 y 0,92 frente a 0,880), manteniendo semilla, datos,
particiones e hiperparámetros idénticos. Se descartaron como causa la precisión
\textit{TF32} (confirmado en tiempo de ejecución que
...
\textit{ranking} sobre un conjunto de validación reducido (81~grafos, entre 4 y 5
cadenas de ataque por pliegue), amplificando el efecto sobre P@5 y F1 macro. Se
incorporaron banderas de determinismo específicas de CUDA
...
a priori sin búsqueda sistemática (Sección~\ref{sec:capa2_diseno}).
